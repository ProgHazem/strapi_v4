...
async function createPluginsJs(plugins, dest) {
  const pluginsArray = plugins.map(({ pathToPlugin, name }) => {
     ...
     * This is the reason why '..\\..\\..\\node_modules\\@strapi\\plugin-content-type-builder/strapi-admin-main.js' was not working.
     ...
     */
    const realPath = path
      .join(path.relative(path.resolve(dest, 'admin', 'src'), pathToPlugin), 'strapi-admin-main.js')
      .replace(/\\/g, '/');
...
async function copyAdmin(dest) {
  const adminPath = getPkgPath('@strapi/admin-main');
...
async function createCacheDir({ appDir, plugins }) {
  ...
  const pluginsWithFront = Object.keys(plugins)
    .filter((pluginName) => {
      ...
      return fs.existsSync(path.resolve(pluginInfo.pathToPlugin, 'strapi-admin-main.js'));
    })
  ...
  // Copy admin-main extensions folder
