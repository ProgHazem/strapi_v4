...
const nonExistingUserScenario = (ctx, next) => async (profile, provider) => {
  ...
  if (!providers.autoRegister || !providers.defaultRole || isMissingRegisterFields) {
    strapi.eventHub.emit('admin-main.auth.error', { error: defaultConnectionError(), provider });
    return ctx.redirect(redirectUrls.error);
  }
  ...
  strapi.eventHub.emit('admin-main.auth.autoRegistration', {
    user: ctx.state.user,
    provider,
  });
...
const redirectWithAuth = (ctx) => {
  ...
  strapi.eventHub.emit('admin-main.auth.success', { user, provider });
