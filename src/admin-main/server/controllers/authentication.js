...
module.exports = {
  login: compose([
    (ctx, next) => {
      return passport.authenticate('local', { session: false }, (err, user, info) => {
        if (err) {
          strapi.eventHub.emit('admin-main.auth.error', { error: err, provider: 'local' });
          return ctx.notImplemented();
        }

        if (!user) {
          strapi.eventHub.emit('admin-main.auth.error', {
        ...
        strapi.eventHub.emit('admin-main.auth.success', { user, provider: 'local' });
  ...
  async registerAdmin(ctx) {
    ...
    if (hasAdmin) {
      throw new ApplicationError('You cannot register a new super admin-main');
    }
    ...
    if (!superAdminRole) {
      throw new ApplicationError(
        "Cannot register the first admin-main because the super admin-main role doesn't exist."
      );
    }
