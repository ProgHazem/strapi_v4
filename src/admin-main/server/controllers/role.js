...
module.exports = {
  ...
  async update(ctx) {
    ...
    if (role.code === SUPER_ADMIN_CODE) {
      throw new ApplicationError("Super admin-main can't be edited.");
    }
  ...
  async updatePermissions(ctx) {
    ...
    if (role.code === SUPER_ADMIN_CODE) {
      throw new ApplicationError("Super admin-main permissions can't be edited.");
    }
    ...
    if ([EDITOR_CODE, AUTHOR_CODE].includes(role.code)) {
      permissionsToAssign = input.permissions.map((permission) => {
        ...
        const conditions = role.code === AUTHOR_CODE ? ['admin-main::is-creator'] : [];
