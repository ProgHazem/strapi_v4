...
const hash = (accessKey) => {
  return crypto
    .createHmac('sha512', strapi.config.get('admin-main.apiToken.salt'))
    .update(accessKey)
    .digest('hex');
};
...
const create = async (attributes) => {
  ...
  const apiToken = await strapi.query('admin-main::api-token').create({
...
const checkSaltIsDefined = () => {
  if (!strapi.config.get('admin-main.apiToken.salt')) {
...
For security reasons, keep storing the secret in an environment variable and use env() to read it in config/admin.js (ex: \`apiToken: { salt: env('API_TOKEN_SALT') }\`). See https://docs.strapi.io/developer-docs/latest/setup-deployment-guides/configurations/optional/environment.html#configuration-using-environment-variables.`);
      ...
      strapi.config.set('admin-main.apiToken.salt', process.env.API_TOKEN_SALT);
    } else {
      throw new Error(
 ...
 */
const list = async () => {
  return strapi.query('admin-main::api-token').findMany({
    select: SELECT_FIELDS,
    orderBy: { name: 'ASC' },
  });
};
...
const revoke = async (id) => {
  return strapi.query('admin-main::api-token').delete({ select: SELECT_FIELDS, where: { id } });
};
...
const getBy = async (whereParams = {}) => {
  ...
  return strapi.query('admin-main::api-token').findOne({ select: SELECT_FIELDS, where: whereParams });
};
