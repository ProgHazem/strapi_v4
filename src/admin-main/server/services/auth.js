...
const checkCredentials = async ({ email, password }) => {
  const user = await strapi.query('admin-main::user').findOne({ where: { email } });
...
const forgotPassword = async ({ email } = {}) => {
  const user = await strapi.query('admin-main::user').findOne({ where: { email, isActive: true } });
  ...
  // Send an email to the admin-main.
  ...
  return strapi
    ...
    .sendTemplatedEmail(
      {
        to: user.email,
        from: strapi.config.get('admin-main.forgotPassword.from'),
        replyTo: strapi.config.get('admin-main.forgotPassword.replyTo'),
      },
      strapi.config.get('admin-main.forgotPassword.emailTemplate'),
      {
        url,
