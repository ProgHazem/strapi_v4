...
const create = async (attributes) => {
  ...
  const createdUser = await strapi.query('admin-main::user').create({ data: user, populate: ['roles'] });
...
const updateById = async (id, attributes) => {
  // Check at least one super admin-main remains
  if (_.has(attributes, 'roles')) {
    ...
    if (lastAdminUser && willRemoveSuperAdminRole) {
      throw new ValidationError('You must have at least one user with super admin-main role.');
    }
  }

  // cannot disable last super admin-main
  ...
  if (_.has(attributes, 'password')) {
    ...
    return strapi.query('admin-main::user').update({
      where: { id },
      data: {
        ...attributes,
...
/**
 * Reset a user password by email. (Used in admin-main:reset CLI)
 * @param {string} email - user email
 * @param {string} password - new password
 */
const resetPasswordByEmail = async (email, password) => {
  const user = await strapi.query('admin-main::user').findOne({ where: { email }, populate: ['roles'] });
...
/**
 * Check if a user is the last super admin-main
...
const exists = async (attributes = {}) => {
  return (await strapi.query('admin-main::user').count({ where: attributes })) > 0;
};
...
const findRegistrationInfo = async (registrationToken) => {
  const user = await strapi.query('admin-main::user').findOne({ where: { registrationToken } });
...
const register = async ({ registrationToken, userInfo }) => {
  const matchingUser = await strapi.query('admin-main::user').findOne({ where: { registrationToken } });
...
const findOne = async (id, populate = ['roles']) => {
  return strapi.entityService.findOne('admin-main::user', id, { populate });
};
...
const findOneByEmail = async (email, populate = []) => {
  return strapi.query('admin-main::user').findOne({
    where: { email },
    populate,
  });
};
...
const findPage = async (query = {}) => {
  ...
  return strapi.entityService.findPage('admin-main::user', enrichedQuery);
};
...
const deleteById = async (id) => {
  // Check at least one super admin-main remains
  const userToDelete = await strapi.query('admin-main::user').findOne({
    where: { id },
    populate: ['roles'],
  });
  ...
  return strapi.query('admin-main::user').delete({ where: { id }, populate: ['roles'] });
};
...
const deleteByIds = async (ids) => {
  // Check at least one super admin-main remains
  const superAdminRole = await getService('role').getSuperAdminWithUsersCount();
  const nbOfSuperAdminToDelete = await strapi.query('admin-main::user').count({
    where: {
      id: ids,
  ...
  const deletedUsers = [];
  for (const id of ids) {
    const deletedUser = await strapi.query('admin-main::user').delete({
      where: { id },
      populate: ['roles'],
    });
...
const countUsersWithoutRole = async () => {
  return strapi.query('admin-main::user').count({
    where: {
      roles: {
        id: { $null: true },
      },
...
const count = async (where = {}) => {
  return strapi.query('admin-main::user').count({ where });
};
...
const assignARoleToAll = async (roleId) => {
  const users = await strapi.query('admin-main::user').findMany({
    select: ['id'],
    where: {
      roles: { id: { $null: true } },
    },
...
const getLanguagesInUse = async () => {
  const users = await strapi.query('admin-main::user').findMany({ select: ['preferedLanguage'] });
