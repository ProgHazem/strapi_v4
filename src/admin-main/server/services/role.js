...
const create = async (attributes) => {
  ...
  return strapi.query('admin-main::role').create({ data: rolesWithCode });
};
...
const findOne = (params = {}, populate) => {
  return strapi.query('admin-main::role').findOne({ where: params, populate });
};
...
const findOneWithUsersCount = async (params = {}, populate) => {
  const role = await strapi.query('admin-main::role').findOne({ where: params, populate });
...
const find = (params = {}, populate) => {
  return strapi.query('admin-main::role').findMany({ where: params, populate });
};
...
const findAllWithUsersCount = async (populate) => {
  const roles = await strapi.query('admin-main::role').findMany({ populate });
...
const update = async (params, attributes) => {
  ...
  return strapi.query('admin-main::role').update({ where: params, data: sanitizedAttributes });
};
...
const exists = async (params = {}) => {
  const count = await strapi.query('admin-main::role').count({ where: params });
  return count > 0;
};
...
const count = async (params = {}) => {
  return strapi.query('admin-main::role').count(params);
};
...
const checkRolesIdForDeletion = async (ids = []) => {
  ...
  if (superAdminRole && stringIncludes(ids, superAdminRole.id)) {
    throw new ApplicationError('You cannot delete the super admin-main role');
  }
...
const deleteByIds = async (ids = []) => {
  ...
  const deletedRoles = [];
  for (const id of ids) {
    const deletedRole = await strapi.query('admin-main::role').delete({ where: { id } });
...
const getUsersCount = async (roleId) => {
  return strapi.query('admin-main::user').count({ where: { roles: { id: roleId } } });
};

/** Returns admin-main role
 * @returns {Promise<role>}
 */
const getSuperAdmin = () => findOne({ code: SUPER_ADMIN_CODE });

/** Returns admin-main role with userCount
 * @returns {Promise<role>}
 */
...
const createRolesIfNoneExist = async () => {
  ...
  const authorPermissions = editorPermissions
    ...
    .map((permission) =>
      permissionDomain.create({ ...permission, conditions: ['admin-main::is-creator'] })
    );
...
const getDefaultPluginPermissions = ({ isAuthor = false } = {}) => {
  const conditions = isAuthor ? ['admin-main::is-creator'] : [];
...
const displayWarningIfNoSuperAdmin = async () => {
  ...
  if (!superAdminRole) {
    strapi.log.warn("Your application doesn't have a super admin-main role.");
  } else if (someUsersExists && superAdminRole.usersCount === 0) {
    strapi.log.warn("Your application doesn't have a super admin-main user.");
  }
};
...
/**
 * Reset super admin-main permissions (giving it all permissions)
 * @returns {Promise<>}
 */
...
/**
 * Check if a user object includes the super admin-main role
 * @param {object} user
 * @return {boolean}
 */
