...
const deleteByRolesIds = async (rolesIds) => {
  const permissionsToDelete = await strapi.query('admin-main::permission').findMany({
    select: ['id'],
    where: {
      role: { id: rolesIds },
    },
...
const deleteByIds = async (ids) => {
  for (const id of ids) {
    await strapi.query('admin-main::permission').delete({ where: { id } });
  }
};
...
const createMany = async (permissions) => {
  ...
  for (const permission of permissions) {
    const newPerm = await strapi.query('admin-main::permission').create({ data: permission });
...
const findMany = async (params = {}) => {
  const rawPermissions = await strapi.query('admin-main::permission').findMany(params);
...
const cleanPermissionsInDatabase = async () => {
  ...
  const total = await strapi.query('admin-main::permission').count();
...
const ensureBoundPermissionsInDatabase = async () => {
  ...
  const editorRole = await strapi.query('admin-main::role').findOne({
