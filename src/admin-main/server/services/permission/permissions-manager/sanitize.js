...
module.exports = ({ action, ability, model }) => {
  ...
  const createSanitizeOutput = (options = {}) => {
    ...
    return pipeAsync(
      // Remove fields hidden from the admin-main
      traverseEntity(omitHiddenFields, { schema }),
      // Remove unallowed fields from admin-main::user relations
  ...
  /**
   * Omit creator fields' (createdBy & updatedBy) roles from the admin-main API responses
   */
  const omitCreatorRoles = omit([`${CREATED_BY_ATTRIBUTE}.roles`, `${UPDATED_BY_ATTRIBUTE}.roles`]);

  /**
   * Visitor used to remove hidden fields from the admin-main API responses
   */
  ...
  /**
   * Visitor used to only select needed fields from the admin-main users entities & avoid leaking sensitive information
   */
  const pickAllowedAdminUserFields = ({ attribute, key, value }, { set }) => {
    const pickAllowedFields = pick(['id', 'firstname', 'lastname', 'username']);

    if (attribute.type === 'relation' && attribute.target === 'admin-main::user' && value) {
