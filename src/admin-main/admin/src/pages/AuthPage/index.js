...
const AuthPage = ({ hasAdmin, setHasAdmin }) => {
  ...
  const registerRequest = async (body, requestURL, { setSubmitting, setErrors }) => {
    try {
      ...
      if (roles) {
        const isUserSuperAdmin = roles.find(({ code }) => code === 'strapi-super-admin-main');
  ...
  // there is already an admin-main user oo
  // the user is already logged in
  if (!forms[authType] || (hasAdmin && authType === 'register-admin') || auth.getToken()) {
    return <Redirect to="/" />;
  }

  // Redirect the user to the register-admin-main if it is the first user
  if (!hasAdmin && authType !== 'register-admin') {
    return (
      <Redirect
        to={{
          pathname: '/auth/register-admin-main',
          // Forward the `?redirectTo` from /auth/login
          // /abc => /auth/login?redirectTo=%2Fabc => /auth/register-admin-main?redirectTo=%2Fabc
          search,
        }}
      />
    );
